...
-- 4.6 전체 데이터 수를 구하여라
SELECT count(*) AS '전체 데이터 수'
FROM superhero;

-- 조건문과 함께 활용하기
-- 4.7 직업이 악당인 사람의 수
SELECT count(직업) AS '악당 수'
FROM superhero
WHERE 직업 = '악당';
...
-- 4.8 전체 평균 나이를 구하여라.
SELECT avg(나이) AS '마블 영웅 평균 나이'
FROM superhero;

-- 그룹 별 계산(GROUP BY)
-- 4.9 각 소속회사의 평균 나이를 구하여라
SELECT 소속회사, avg(나이) AS '소속회사 평균 나이'
FROM superhero
GROUP BY 소속회사;

-- 4.10 소속회사 별로 그룹화 하여, 40살 이상인 데이터만 조회하여라.
SELECT 소속회사, avg(나이) AS '40살 이상'
FROM superhero
WHERE 나이 >= 40
GROUP BY 소속회사;
